...
scwin.btn_confirm_onclick = function () {
  const data = grd_program.getCheckedJSON("chk");
  if (data.length == 1) {
    $c.win.closePopup($p, data[0]);
  } else if (data.length == 0) {
    $c.win.alert($p, '프로그램이 선택되지 않았습니다.');
  }
};
...
scwin.grd_program_oncelldblclick = function (row, col, colId) {
  $c.win.closePopup($p, grd_program.getRowJSON(row));
};
